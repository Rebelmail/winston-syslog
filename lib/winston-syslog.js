...
class Syslog extends Transport {
  ...
  parseProtocol(protocol = this.protocol) {
    ...
    if (this.protocolType !== 'tcp' && this.tls) {
      throw new Error('`options.tls` requires a tcp protocol');
    }
  ...
  connect(callback) {
    ...
    if (this.tls) {
      this.socket = new tls.TLSSocket(this.socket, Object.assign({}, this.tls));
    }
  ...
  setupEvents() {
    ...
    this.socket.on(readyEvent, () => {
      if (this.tls && !this.socket.authorized) {
